...
def processing_worker():
    ...
    while not thread_event.is_set():
        ...
        arrs = []
        while not q.empty():
            arrs.append(q.get()) # grab all the blocks until there's no more.  Pray that we do this faster than the stream puts blocks in there
        if arrs:
            ...
            model_output, midi_data, note_events = predict(audio_path=filename, 
            ...
            if len(midi_data.instruments) > 0:
                ...
                for note in instrument.notes:
                    ...
                    print(f"  Pitch: {note.pitch} ({note_name}), Velocity: {note.velocity}, Start: {note.start:.2f}s, End: {note.end:.2f}s")
        if print_debug:
